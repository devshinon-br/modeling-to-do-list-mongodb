**1. Estatísticas do Banco de Dados antes da otimização**

Para obter as estatísticas do banco de dados antes de criar índices, é possível usar o seguinte comando no shell do MongoDB:

``` shell
db.projects.stats()
```

Este comando fornecerá estatísticas gerais sobre a coleção, incluindo o tamanho total, número de documentos, entre outras informações.

**2. Criação de Índices**

``` shell
# Índice para o campo "createdAt"
db.projects.createIndex({ "createdAt": 1 })

# Índice para o campo "tasks.status"
db.projects.createIndex({ "tasks.status": 1 })
```

Os valores "1" indicam ordem ascendente. 

**3. Estatísticas do Banco de Dados depois da otimização**

Após criar os índices, deve-se executar novamente o comando `stats()` para obter as estatísticas atualizadas:

``` shell
db.projects.stats()
```

**4. Teste a diferença de desempenho**

``` shell
# Consulta antes da criação dos índices
db.projects.find({ "createdAt": "2022-11-11" })

# Consulta após a criação dos índices
db.projects.find({ "createdAt": "2022-11-11" })
  .hint({ "createdAt": 1 })
```

A consulta com o `hint()` deve ser muito mais rápida do que a consulta sem ele. 

### O `hint()`especifica um índice para ser usado em uma consulta.


## Resultados

### stats() Antes de criar os índices
``` shell
{
  ok: 1,
  capped: false,
  wiredTiger: {
    ...
    type: 'file',
    uri: 'statistics:table:collection-8-2898482407592294021',
    LSM: {
    ...
    cache: {
      'bytes currently in the cache': 23672,
      'bytes dirty in the cache cumulative': 882,
      'bytes read into cache': 0,
      'bytes written from cache': 18420,
 ...
 0,
      ...
      'pages requested from the cache': 54,
    ...
    cursor: {
      ...
      'cache cursors reuse count': 24,
      'close calls that result in cache': 25,
      'create calls': 3,
      'cursor next calls that skip due to a globally visible history store tombstone': 0,
      'cursor next calls that skip greater than or equal to 100 entries': 0,
      'cursor next calls that skip less than 100 entries': 714,
      'cursor prev calls that skip due to a globally visible history store tombstone': 0,
      'cursor prev calls that skip greater than or equal to 100 entries': 0,
      'cursor prev calls that skip less than 100 entries': 1,
      'insert calls': 30,
      'insert key and value bytes': 18208,
      modify: 0,
...
### stats() Depois de criar os índices
``` shell
{
  ok: 1,
  capped: false,
  wiredTiger: {
    ...
    btree: {
      'btree checkpoint generation': 152,
      'btree clean tree checkpoint expiration time': Long("9223372036854775807"),
      'btree compact pages reviewed': 0,
      'btree compact pages rewritten': 0,
      'btree compact pages skipped': 0,
 ...
 0,
    ...
    cursor: {
      ...
      'Total number of times a search near has exited due to prefix config': 0,
      'bulk loaded cursor insert calls': 0,
      'cache cursors reuse count': 27,
      'close calls that result in cache': 29,
      'create calls': 4,
      'cursor next calls that skip due to a globally visible history store tombstone': 0,
      'cursor next calls that skip greater than or equal to 100 entries': 0,
      'cursor next calls that skip less than 100 entries': 778,
      'cursor prev calls that skip due to a globally visible history store tombstone': 0,
      'cursor prev calls that skip greater than or equal to 100 entries': 0,
      'cursor prev calls that skip less than 100 entries': 1,
      'insert calls': 30,
  ...
  totalSize: 86016,
  indexSizes: { _id_: 20480, createdAt_1: 20480, 'tasks.status_1': 20480 },
  avgObjSize: 605,
  ns: 'todo_list.projects',
  nindexes: 3,
  scaleFactor: 1
